        ...
        if (objList != null && objList.size() > 0) {
            ...
            try {
                ...
                for (int j = 0; j < objList.size(); j++) {
                    Budget projectBean = (Budget) objList.get(j);
                    List<String> list = new ArrayList<>();
                    String budgetType = DBManger.getInstance(c).getBudgetTypeByID(projectBean.getBudegetTypeId());
                    list.add(budgetType);
                    list.add(projectBean.getNum());
                    list.add(projectBean.getDate());
